...
| Task | Approach |
|------|----------|
| Read/analyze content | `markitdown` or unpack for raw XML |
...
- Cell `margins` are internal padding; they reduce the content area and do not add to the cell width
...
- **Replace entire `<w:r>` elements**: When adding tracked changes, replace the whole `<w:r>...</w:r>` block with `<w:del>...<w:ins>...` as siblings. Do not inject tracked change tags inside a run.
...
**Restoring another author's deletion** - add the insertion after (do not modify their deletion):
```xml
